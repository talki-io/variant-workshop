...
| 账号 | 角色 | 能看到什么 |
| --- | --- | --- |
| `admin` | 管理员 | 文案生成 / 新闻库 / **账号管理** / **用户管理** / **菜单管理** / 消耗看板 / 抓取与配额 / 模型管理 |
| `editor` | 素材员 | 文案生成 / 新闻库 / **账号管理**；其余 admin 菜单隐藏，直接打这些 admin 端点后端返回 **403** |

> **侧栏菜单数据驱动**（表 `menu_item`，见「菜单管理」）：侧栏与面包屑实时读该表渲染，管理员可增删改现有页面的菜单项（名称/图标/排序/启用/按角色可见）。`visible_roles` 治理侧栏可见 + 前端路由可达（`RequireMenuAccess`）；真正的数据安全仍靠后端各敏感端点的 `require_admin` 兜底。`/users`、`/menus` 为 `locked` 核心项（禁删）且路由用 `RequireAdmin` 硬门禁，防误配自锁。
>
> **用户管理（admin）**：界面化增删改用户、改角色、重置密码、启用/停用（软删——停用后不能登录、现存 token 失效）。改名跨 `generation_session`/`token_usage`/`telemetry_event` 三张 name 键表级联更新；删除级联清该用户的账号+参考文案+生成会话，保留历史用量。守卫：不能停用/删除自己或最后一个管理员。CLI `python -m app.create_user` 仍可用（生产开口子）。
>
> **账号管理对所有用户开放，但按创建者隔离**：每个用户只看得到、管得了自己新增的账号（`Tone`）及其参考爆款样本（`StyleSample`），用户之间互不可见、互不干扰。种子账号 `t1-t4` 归 `admin`，`editor` 初始无账号、登录后自建。生成/重生成会校验账号归属，不能借用他人账号（及其样本）。
>
